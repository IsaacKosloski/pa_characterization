...
Gráficos incluídos:
    1. Diagrama de Constelação (IQ Plot)
       - Compara sinal original, distorcido pelo PA e linearizado.
    2. Curva AM-AM
...
_COLORS = {
    ...
    "distorted":   "#F44336",  # vermelho
    "linearized":  "#4CAF50",  # verde
...
def plot_constellation(
    ...
    O diagrama IQ mostra a parte real (eixo I) vs a parte
    imaginária (eixo Q) de cada amostra. Num PA ideal:
        - Os pontos de Y devem coincidir com os de X
          (apenas escalados por G).
    Distorções não-lineares causam espalhamento (EVM alto) e
    rotação dos pontos.

    Parâmetros
    ----------
    X_original   : sinal de entrada ideal (referência)
    Y_distorted  : saída do PA não-linear (modelo original)
    Y_linearized : saída com betas otimizados pelo GA
    ...
    datasets = [
        ...
        (Yd, "PA Distorcido\n(Modelo Base)",  _COLORS["distorted"],  axes[1]),
        (Yl, "PA Linearizado\n(GA Otimizado)",_COLORS["linearized"], axes[2]),
    ]
    ...
    if save_path:
        ...
        print(f"\nConstelação salva em: {save_path}")
...
def _compute_evm(X_ref: np.ndarray, Y_test: np.ndarray) -> float:
    ...
    Normaliza pelo PA esperado (X escalado pelo ganho médio)
    para ter uma métrica relativa ao nível do sinal.
...
def plot_am_am_pm(
    ...
    """
    amp_in = np.abs(X)
    gain_d = np.abs(Y_distorted)  / (amp_in + 1e-10)
    gain_l = np.abs(Y_linearized) / (amp_in + 1e-10)

    phase_d = np.angle(Y_distorted,  deg=True) - np.angle(X, deg=True)
    phase_l = np.angle(Y_linearized, deg=True) - np.angle(X, deg=True)
    ...
    # -- AM-AM
    ax1.scatter(amp_in, gain_d, c=_COLORS["distorted"],  alpha=0.3, s=3, label="Distorcido")
    ax1.scatter(amp_in, gain_l, c=_COLORS["linearized"], alpha=0.3, s=3, label="Linearizado")
    ...
    # -- AM-PM
    ax2.scatter(amp_in, phase_d, c=_COLORS["distorted"],  alpha=0.3, s=3, label="Distorcido")
    ax2.scatter(amp_in, phase_l, c=_COLORS["linearized"], alpha=0.3, s=3, label="Linearizado")
...
def plot_ga_convergence(
    ...
    Mostra as curvas de mínimo, média e desvio padrão, permitindo
    avaliar:
    ...
    ax.plot(gen, rm_min, color=_COLORS["linearized"], linewidth=2, label="RMSE mínimo (melhor)")
...
def plot_full_dashboard(
    ...
    for col, (sig, label, color) in enumerate(zip(
        [X[idx], Y_distorted[idx], Y_linearized[idx]],
        ["Entrada X[n]", "PA Distorcido", "PA Linearizado"],
        [_COLORS["original"], _COLORS["distorted"], _COLORS["linearized"]],
    )):
    ...
    amp = np.abs(X)
    ax_am.scatter(amp, np.abs(Y_distorted),  c=_COLORS["distorted"],  alpha=0.3, s=3, label="Distorcido")
    ax_am.scatter(amp, np.abs(Y_linearized), c=_COLORS["linearized"], alpha=0.3, s=3, label="Linearizado")
    ...
    if logbook:
        ...
        ax_ga.plot(gen, rm_min, color=_COLORS["linearized"], linewidth=2, label="RMSE min")
