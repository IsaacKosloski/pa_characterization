...
from src.models.trainer_lstm import TrainerLSTM
from src.core.dataset import Dataset
from src.core.sliding_window import SlidingWindowDataset
from torch.utils.data import DataLoader
from itertools import product

class GridSearchMetricsAnalyser():
    ...
    def __init__(self, save_path):
        ...
        (self.save_path/"models").mkdir(parents=True, exist_ok=True)
    ...
    def update_results(self, rmse, evm, avg_loss, model, params):
        for k, metric in zip(self.results.keys(),(rmse, evm, avg_loss)):
            if self.comp(metric, self.results[k]['value']):
                self.results[k]['value'] = metric
                self.results[k]['hyperparameters'] = self.get_params_dict(params)
                torch.save(model.state_dict(), self.save_path/"models"/f"best_{k}_lstm.pkl")
    ...
    def update_history(self, key_hyperparams, values):
        ...
        key = str(key_hyperparams[:-1])
        n_epochs = key_hyperparams[-1]

        if (key not in self.history) or (n_epochs not in self.history[key]):
            history_element = {
                n_epochs: {
            ...
            self.history[key] = history_element
        else:
    ...
    def save(self):
        ...
        with open(f"{self.save_path}/best_results.json", "w") as f:
            json.dump(self.results, f, indent=2)

        with open(f"{self.save_path}/history.json", "w") as f:
            json.dump(self.history, f, indent=2)

class GridSearchLSTM():
    def __init__(self, train, val, test, param_grid, device):
        self.train = train
        self.val = val
        self.test = test
        ...
        self.test_norm = test.normalize(test, self.scaler_x, self.scaler_y)
        self.device = device
        list(product(self.param_grid["windows_size"],self.param_grid["hidden_size"],self.param_grid["num_layers"],self.param_grid["learning_rate"],self.param_grid["dropout"],self.param_grid["batch_size"],self.param_grid["epoch"]))
    ...
    def run(self, save_path):
        ...
        result_analyser = GridSearchMetricsAnalyser(save_path)
        ...
        for i, (ws, hs, nl, lr, dr, bs, n_epochs) in enumerate(params_combination):
            ...
            trainer = TrainerLSTM(model,nn.MSELoss(),lr,self.scaler_y,device=self.device)
