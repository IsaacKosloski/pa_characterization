"""
Configuração central do modelo NARX para o PA.

O modelo NARX (Nonlinear AutoRegressive with eXogenous inputs)
representa matematicamente como o PA transforma o sinal de entrada
X[n] (complexo) no sinal de saída Y[n] (complexo), capturando:

  1. MEMÓRIA LINEAR (matrizes autorregressivas A_1, A_2):
     efeitos de realimentação da saída passada.

  2. NÃO-LINEARIDADE (betas com deg > 1):
     a compressão de ganho e a geração de harmônicas do PA.

  3. EFEITOS DE MEMÓRIA DA ENTRADA (betas com lags):
     dispersão temporal causada por capacitâncias parasitas,
     efeitos térmicos e de armadilhas do transistor.

Estrutura da equação:
  Y(t) = intercepto
       + A_1 * Y(t-1) + A_2 * Y(t-2)     <- parte AR (autorregressiva)
       + sum(beta_k * phi_k(X(t), lags))   <- parte X exógena (NARX)

Onde phi_k são os regressores: Xreal^deg, Ximg^deg, com lags 0,1,2.
"""
...
class Intercepts:
    ...
    Fisicamente: desbalanceamento de tensão no ponto de operação do PA.
    ...
    def to_array(self) -> np.ndarray:
        """" Retorna interceptos como vetor coluna [Yreal, Yimg]"""
        return np.array([self.Yreal, self.Yimg])

# --- Matrizes Autoregressivas (A_1, A_2)
@dataclass
class AutoregressiveMatrix:
    ...
    Fisicamente: modela a memória do PA causada por efeitos de self-heating, traps de cargaeimpedância parasitasquefazemo ganho atual dependerde amplificações anteriores.

    Valores pequenos (<<1) indicam que a memória é fraca, ou seja, o PA não tem muita histerese, típico de PAs GaN bem projetados.
    """

    # Linha 1 da matriz: contribuição para Yreal(t)
    Yreal_to_Yreal: float = 0.0
    Yimg_to_Yreal:  float = 0.0
    # Linha 2 da matriz: contribuição para Yimg(t)
    ...
    def to_matrix(self) -> np.ndarray:
        """
        :return:
            Matrix 2x2: A * Y(t-k) contribui para Y(t)
        """
        return np.array([
            [self.Yreal_to_Yreal, self.Yimg_to_Yreal],
            [self.Yreal_to_Yimg, self.Yimg_to_Yimg]
        ])

# --- Coeficientes Beta (Variáveis Exógenas)
@dataclass
class ExogenousBetas:
    """
    Coeficientes que mapeiam a entrada X[n] (complexa) na saída Y[n}.

    Convenção de nomes:
        beta.X{componente}_lag{k}_deg{d}.Y{saída}
    Onde:
    ...
    Fisicamente:
        - deg=1 (linear): ganho nominal do PA → ~23.45 ≈ 27.4 dB
        - deg=2 (quadrático): distorção de 2ª harmônica (HD2), IM2
        - deg=3 (cúbico): compressão de ganho (AM-AM), rotação de fase
          (AM-PM), IM3 — os mais críticos para linearidade!
    ...
    def to_dict(self) -> Dict[str, float]:
        """ Serializa todos os betas em dicionários chave->valor."""
        return asdict(self)
    def to_array(self) -> np.ndarray:
        """
        Vetoriza os betas em array numpy.
        Ordem: primeiro todos os betas de Yreal, depois de Yimg.
        Usado pelo Algoritmo Genético com cromossomo.
        """
        return np.array(list(self.to_dict().values()), dtype=float)
    @classmethod
    def from_array(cls, beta_array: np.ndarray) -> "ExogenousBetas":
        """
        Reconstrói ExeogenousBetas a partir de um array (cromossomo do GA).
        Exige que a ordem corresponda exatamente à de to_array().
        :param beta_array:
        :return:
        """
        fields = list(cls.__dataclass_fields__.keys())
        if len(beta_array) != len(fields):
            raise ValueError(f"Array tem {len(beta_array)} elementos, mas o modelo exige {len(fields)}.")
        return cls(**dict(zip(fields, beta_array)))
...
class NARXModelConfig:
    ...
    def gain_linear_dB(self) -> float:
        ...
        Deriva dos betas de primeiro grau (Xreal->Yreal e Ximg->Yimg),
        assumindo simetria do canal (caso ideal: beta_cross ≈ 0).
        """
