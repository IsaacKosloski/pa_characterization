"""
Simulador de inferência do modelo NARX.

Responsável por receber X[n] (sinal de entrada complexo) e produzir Ŷ[n] (sinal de saída predito complexo), executando a equação NARX (VARMAX com não-linearidades e sem MA) completa com memória autorregressiva e regressores polinomiais da entrada exógena.
"""

import numpy as np
from typing import Tuple
from .model_config import NARXModelConfig

class NARXEngine:
    """
    Executa a predição amostra-a-amostra do modelo.
    """
    ...
    def __init__(self, config: NARXModelConfig):
        ...
        self._b = config.betas  # ExogenousBetas
    ...
    def _build_regressors(
        ...
        Ordem (18 regressores para cada saída = 36 total, mas
        os betas são separados por saída, então retornamos 18):

            [Xr, Xi,                      ← deg 1 (lag 0)
             Xr_l1, Xi_l1,               ← deg 1 (lag 1)
             Xr_l2, Xi_l2,               ← deg 1 (lag 2)
             Xr^2, Xr_l1^2, Xr_l2^2,    ← deg 2
             Xi^2, Xi_l1^2, Xi_l2^2,
             Xr^3, Xr_l1^3, Xr_l2^3,    ← deg 3
             Xi^3, Xi_l1^3, Xi_l2^3]
        """
        return np.array([
            # -- Lineares
            xr0, xi0,
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        # A1 e A2 da config (transposta para multiplicação por vetor coluna)
        A1 = self._A1  # shape (2,2)
        A2 = self._A2  # shape (2,2)
        c = self._c  # shape (2,)

        for t in range(N):
            # -- Regressores de X (com padding zero para lags iniciais, evitando anti-causalidade falsa)
            xr0 = Xr[t]
            xi0 = Xi[t]
            ...
            # -- Parte autorregressiva: A1*Y(t-1) + A2*Y(t-2)
            ...
            ar_contrib = A1 @ Y_prev1 + A2 @ Y_prev2  # shape (2,)
    ...
    def rmse(Y_true: np.ndarray, Y_pred: np.ndarray) -> float:
        """
        RMSE sobre a envoltória complexa.
        Calcula o erro em ambas as componentes (I e Q) simultaneamente.
    ...
    def nmse(Y_true: np.ndarray, Y_pred: np.ndarray) -> float:
        """
        NMSE (Normalized Mean Square Error) em dB.
...
def compute_gain(X: np.ndarray, Y: np.ndarray) -> np.ndarray:
    ...
    :return G : np.ndarray complex (ganho complexo por amostra)
    '"""
